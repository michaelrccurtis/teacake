...
test('non strings should throw', () => {
  ...
  expect(() => field.serialize({ attr: 'test', obj: { test: Date.now() }})).toThrow(FieldValidationError);
  expect(() => field.deserialize({ attr: 'test', data: { test: Date.now() }})).toThrow(FieldValidationError);
});
