...
test('basic deserialization', () => {
  const field = new Field({required: false});
  expect(field.deserialize({ attr: 'test', data: { test: 'TEST' } })).toBe('TEST');
...
test('ignore is respected', () => {
  ...
  expect(field.deserialize({ attr: 'test', data: { test: 'IGNORE_ME' } })).toBe(MISSING);
  expect(field.deserialize({ attr: 'test', data: { test: 'FIND_ME' } })).toBe('FIND_ME');

  const field2 = new Field({default: 'ME_INSTEAD', ignore: [null, 'IGNORE_ME']});
  expect(field2.serialize({ attr: 'test', obj: { test: null } })).toBe('ME_INSTEAD');
  expect(field2.serialize({ attr: 'test', obj: { test: 'IGNORE_ME' } })).toBe('ME_INSTEAD');
  expect(field2.serialize({ attr: 'test', obj: { test: 'FIND_ME' } })).toBe('FIND_ME');

  const field3 = new Field({missing: 'ME_INSTEAD', ignore: [null, 'IGNORE_ME']});
  expect(field3.deserialize({ attr: 'test', data: { test: null } })).toBe('ME_INSTEAD');
  expect(field3.deserialize({ attr: 'test', data: { test: 'IGNORE_ME' } })).toBe('ME_INSTEAD');
  expect(field3.deserialize({ attr: 'test', data: { test: 'FIND_ME' } })).toBe('FIND_ME');
});
