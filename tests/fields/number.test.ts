...
test('should coerce stringified numbers', () => {
  const field = Fields.Number({required: false});
  expect(field.deserialize({ attr: 'test', data: { test: "1000" }})).toBe(1000);
  expect(field.deserialize({ attr: 'test', data: { test: "2000" }})).toBe(2000);
  expect(field.deserialize({ attr: 'test', data: { test: "3.123" }})).toBe(3.123);
});

test('should not coerce stringified numbers in strict mode', () => {
  const field = Fields.Number({required: false, strict: true});
  expect(() => field.deserialize({ attr: 'test', data: { test: "1000" }})).toThrow(FieldValidationError);
  expect(() => field.deserialize({ attr: 'test', data: { test: "2000" }})).toThrow(FieldValidationError);
  expect(() => field.deserialize({ attr: 'test', data: { test: "3.123" }})).toThrow(FieldValidationError);
});
